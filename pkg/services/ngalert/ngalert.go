...
// If this constanst is lower than the configured minimum evaluation interval then
// this configuratin is returned.
...
// the default interval is returned.
func (ng *AlertNG) getRuleMinIntervalSeconds() int64 {
	if ng.Cfg.AlertingMinInterval <= 0 {
		return defaultIntervalSeconds // if it's not configured; apply default
	}

	if ng.Cfg.AlertingMinInterval%baseIntervalSeconds != 0 {
		ng.Log.Debug("Configurred minimum evaluation interval is not divided exactly by the scheduler interval and it will fallback to default", "alertingMinInterval", ng.Cfg.AlertingMinInterval, "baseIntervalSeconds", baseIntervalSeconds, "defaultIntervalSeconds", defaultIntervalSeconds)
		return defaultIntervalSeconds // if it's invalid; apply default
	}
